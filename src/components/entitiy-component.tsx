...
export const EntityHeader = ({ title, description, newButtonLabel, disable, isCreating, onNew, newButtonHref }: EntityHeaderProps) => {
    return (
        <div className="flex flow-row items-center justify-between gap-x-4">
            ...
            {newButtonHref && !onNew && (
                <Button
                    size="sm"
                    asChild>
                    <Link to={newButtonHref} preload="intent">
                        <PlusIcon className="size-4" />
                        {newButtonLabel}
                    </Link>
                </Button>
